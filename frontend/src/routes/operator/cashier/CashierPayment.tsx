...
export default function CashierPayment() {
  ...
  // Filter the loaded list by containment so the list narrows as the user types.
  const filteredOrders = useMemo(() => {
    if (!orders || !trimmed) return orders;
    return orders.filter((o) => o.orderId.toLowerCase().includes(trimmed));
  }, [orders, trimmed]);
  ...
  async function handleSearch(event: React.FormEvent) {
    ...
    // If the local list has exactly one match, navigate directly without an API call.
    if (filteredOrders && filteredOrders.length === 1) {
      navigate(paths.operator.cashierPaymentOrder(eventId, filteredOrders[0].orderId));
      return;
    }

    // Multiple local matches: let the cashier click one — no need to search.
    if (filteredOrders && filteredOrders.length > 1) return;

    // No local match: try the API (handles orders not in the unpaid list).
    setIsSearching(true);
    try {
      const order = await getOrder(trimmed);
      navigate(paths.operator.cashierPaymentOrder(eventId, order.orderId));
    } catch {
      setSearchError(`No order found for "${query.trim()}".`);
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      >
        ...
        <div className="mt-3 flex gap-3">
          ...
          />
          <Button type="submit" size="lg" disabled={isSearching || !trimmed}>
                ...
                <li key={order.orderId}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-base font-semibold text-accent">{order.orderId}</span>
