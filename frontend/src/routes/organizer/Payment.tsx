...
function InfoTooltip({ text }: { text: string }) {
  return (
    <span className="group relative inline-flex">
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute bottom-full left-0 z-50 mb-1.5 w-56 max-w-[calc(100vw-2rem)] rounded-lg border border-border bg-surface px-3 py-2 text-xs leading-snug font-normal text-text opacity-0 shadow-lg transition-opacity duration-150 group-hover:opacity-100 group-focus-within:opacity-100 sm:left-1/2 sm:-translate-x-1/2"
      >
        {text}
      </span>
    </span>
